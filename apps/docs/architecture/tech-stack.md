...
| ORM / 数据 | Prisma + PostgreSQL | 数据模型、迁移、类型安全 | 可替换 ORM，但 Prisma 扩展承担的审计 / 软删除逻辑需要迁移 |
...
| Shared | `packages/api-common` | 权限常量、请求抽象、共享类型、API 工厂 | 这是跨端协议边界，不建议拆散 |
...
- 前后端共享协议，不共享 UI。
- 后端 route 负责协议，service 负责业务，Prisma 扩展负责数据治理。
