...
- 在 `apps/backend/prisma/` 下对应的 schema 文件增加模型和关系；`schema.prisma` 只保留 `generator / datasource`，共享枚举放在 `enums.prisma`，业务模型按领域放在 `models/*.prisma`
- 如果模型需要统一的 Snowflake ID、操作者字段、软删除和 `Operation` 审计，把模型名加入 `apps/backend/src/lib/prisma.ts`
...
- 异步处理器统一走 `asyncHandler(...)`
- 默认数据库入口使用 `prisma`
- 只有需要原始表语义、关系恢复或显式事务 client 时才使用 `prismaRaw`
- 如果服务层需要一个明确的多步事务块，使用 `runInBackendRuntimeTransaction(...)`
- 如果接口为了协议兼容必须自己输出错误响应，响应写出后调用 `rollbackHandledResponse()` 触发回滚
- 只要数据库操作经过当前请求里的 `prisma / prismaRaw`，框架就会自动生成 `RequestRecord + Operation`，不要在业务接口里再手工写审计日志
- 如果是列表接口，统一返回：
...
- 在 `apps/backend/src/constants/system-permissions.ts` 增加权限码
- 在 `apps/backend/src/services/system-rbac.ts` 中决定是否把页面和行为挂到菜单树
- 如果需要默认角色拥有权限，也在 seed 流程中同步
- 如果这个能力还会开放 websocket 订阅，同时补 `RealtimeTopic` 注册和 seed
...
如果你改到了 `asyncHandler`、`BackendRuntimeContext`、`runInBackendRuntimeTransaction(...)`、`prisma` / `prismaRaw` 这类底层抽象，优先补 `framework/runtime-transaction.test.ts` 这种框架级测试，而不是只补业务接口回归。
如果你改到了请求审计 flush 或审计保留期清理逻辑，再补 `framework/request-audit-retention.test.ts`。
...
2. 在 `apps/backend/src/constants/system-permissions.ts` 增加订阅权限码。
3. 在 `apps/backend/src/topics/*.ts` 注册 `code`、`topicPattern`、`permissionCode` 和需要的 `authorizeSubscription / onSubscribed / onUnsubscribed / onPublished`。
4. 让 `apps/backend/src/services/system-rbac.ts` 把注册项 seed 到 `RealtimeTopic`，形成 `Permission -> RealtimeTopic` 的数据库绑定。
5. 业务侧继续只调用 `publishRealtimeMessage(...)` 或领域 helper，不要在路由或页面里手写授权匹配。
6. 补 `framework/realtime-topic.test.ts` 和 `integration/realtime.test.ts`，验证 topic 覆盖关系、订阅授权和推送行为。
