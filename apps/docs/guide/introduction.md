...
`QC-RBAC` 是一个面向长期演进的认证与授权 Monorepo。当前仓库已经收敛了账号体系、客户端识别、RBAC、菜单路由、OAuth/OIDC、附件、导出、测试和开发文档。
...
- 前后端不希望重复维护客户端类型、请求头、RBAC 类型、topic 规则和 API 类型。
...
| `packages/api-common` | 共享协议层 | 类型、RBAC 协议、客户端枚举、请求封装、realtime 协议、API 工厂 |
...
| RBAC / 菜单 | `apps/backend/src/services/system-rbac.ts`、`apps/backend/src/routes/menus.ts` | `apps/web-frontend/src/stores/menus.ts` | `packages/api-common/src/types/rbac.ts` |
...
| 关系选择器 | 资源 `options` 接口 + `resolve(ids)` 回显接口 | `apps/web-frontend/src/components/form/RelationSelectFormItem.vue` | `OptionSearchPayload` |
...
1. 想知道系统是怎么启动的，先看 `apps/backend/src/main.ts`、`apps/backend/src/app.ts` 和 `apps/web-frontend/src/api/client.ts`。
...
- 系统权限种子在 `apps/backend/src/constants/system-permissions.ts` 维护；前端运行时权限来自 `/api/auth/me` 和 `/api/menus/current`。
- 后端列表接口统一返回 `items + meta`，分页字段统一为 `page` / `pageSize`。
- Web 列表导出统一使用 `createExcelExportHandler + useDownload + ListExportButton`。
- 表单里的关联选择统一使用 `RelationSelectFormItem`，搜索区通过插槽传入，不在组件内部写死，并且后端必须同时提供分页查询和 `resolve(ids)` 回显。
- Uni 页面统一使用自定义组件和 `navigationStyle: 'custom'`。
- 受管模型的 Snowflake ID、操作者字段、请求审计、软删除、删除引用检查统一在 `BackendRuntimeContext + 受管 Prisma` 这一层处理，并自动跟随请求事务。
- 任何有行为变化的改动都要同步测试和文档。
