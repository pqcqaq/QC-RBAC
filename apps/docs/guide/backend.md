...
- `prisma.config.ts`：Prisma CLI 配置，当前通过 `schema: 'prisma'` 指向整个 schema 目录。
- `prisma/schema.prisma`：只保留 `generator` 和 `datasource`。
- `prisma/enums.prisma` 与 `prisma/models/*.prisma`：分别放共享枚举和按领域拆分的模型定义；`migrations/` 继续与 `schema.prisma` 同级。
...
- 启动时直接读取 Prisma Client 内部挂着的 `client._engineConfig.inlineSchema`
  - 也就是当前进程实际使用的那份内联 Prisma schema，运行时会把 `prisma/` 目录下的多文件 schema 合并成这一份字符串
  - 不需要再额外去磁盘读文件
- 解析过程是轻量字符串解析：
...
这些图不是完整 schema 抄录，而是帮助第一次接触项目的人快速建立主关系。真实字段以 `prisma/` 目录下的 Prisma schema 文件为准：`schema.prisma` 保留 `generator / datasource`，共享枚举放在 `enums.prisma`，业务模型按领域放在 `models/*.prisma`。
...
- `/api/attachments/options/images` 和 `/resolve` 返回已完成图片记录，`attachment` 和 `avatar` 两种 kind 都可作为图片选择器数据源
- 图片 option 查询额外支持 `maxSize`，会在后端直接过滤超限图片，避免前端只能做本地提示
...
1. 在 `apps/backend/prisma/` 下对应的 schema 文件增加模型和关系；`schema.prisma` 只保留 `generator / datasource`，共享枚举放在 `enums.prisma`，业务模型按领域放在 `models/*.prisma`。
