...
2. 如果要接实时推送或排查 websocket，先读 [实时通信](/guide/realtime)。
3. 再读 [Web 前端](/guide/web-frontend)，理解控制台页面是如何接后端能力的。
4. 如果要复用现成组件，直接读 [内置组件](/components/)。
5. 如果要改移动端，再读 [Uni 前端](/guide/uni-frontend)。
6. 如果要补测试、查覆盖面或定位回归，直接读 [测试用例](/guide/testing)。
7. 如果要动共享协议或新增客户端类型，直接读 [共享抽象](/guide/shared) 和 [扩展指南](/guide/extension)。
