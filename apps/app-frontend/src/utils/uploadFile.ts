...
type UploadResponseEnvelope<T> = {
  data?: T
}

export interface UploadOptions<T = string> {
  /** 最大可选择的图片数量，默认为1 */
  count?: number
...
export function useFileUpload<T = string>(
  ...
  options: Omit<UploadOptions<T>, 'sourceType' | 'sizeType' | 'count'> = {},
) {
  return useUpload<T>(
    url,
    formData,
...
export function useUpload<T = string>(
  ...
  const startUpload = (tempFilePath: string) => {
    loading.value = true
    progress.value = 0
    uploadFile<T>({
      url,
  ...
  const run = () => {
    ...
    uni.chooseImage({
      ...
      success: (res) => {
        ...
        startUpload(res.tempFilePaths[0])
      },
      fail: (err) => {
...
const parseUploadResponse = <T>(payload: string): T | undefined => {
  const parsedResponse = JSON.parse(payload) as UploadResponseEnvelope<T>
  ...
  try {
    ...
    const uploadTask = uni.uploadFile({
      ...
      success: (uploadFileRes) => {
        ...
        try {
          const parsedData = parseUploadResponse<T>(uploadFileRes.data)
